...
const TabGroup: React.FC<TabGroupProps> = ({ children, className = "" }) => {
    const tabs: React.ReactElement[] = [];
    const panels: React.ReactElement[] = [];

    React.Children.forEach(children, (child) => {
        if (React.isValidElement(child) && child.type && (child.type as any).displayName === "Tab") tabs.push(child);
        else if (React.isValidElement(child)) panels.push(child);
    });
